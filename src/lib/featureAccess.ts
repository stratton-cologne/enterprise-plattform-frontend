        ...
        return {
            ...
            modules:
                typeof parsed?.modules === "object" && parsed.modules !== null
                    ? parsed.modules
                    : {},
            activePluginIds: Array.isArray(parsed?.activePluginIds)
                ? parsed.activePluginIds.map(String)
                : [],
        };
    } catch {
...
export const fetchFeatureAccessSnapshot =
    async (): Promise<FeatureAccessSnapshot> => {
        ...
        const activePluginsRaw = Array.isArray(pluginsResponse?.data)
...
export const ensureFeatureAccessSnapshot = async (
    maxAgeMs = 60_000,
