...
<template>
  <div class="space-y-6">
    <section class="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      <div class="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-zinc-500 dark:text-zinc-400">Persönliche Daten, Profilbild und MFA verwalten.</p>
        </div>
      </div>
      ...
      <form v-else class="grid gap-4 md:grid-cols-2" @submit.prevent="saveProfile">
        <div class="md:col-span-2 flex items-center gap-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-950">
          <div
          ...
          <div class="flex flex-wrap items-center gap-2">
            <label class="cursor-pointer rounded-lg border border-zinc-300 px-3 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
              Profilbild wählen
        ...
        <div class="grid gap-3 md:grid-cols-10">
          ...
          <div
            class="rounded-lg border px-3 py-2 text-sm md:col-span-3"
        ...
        <div class="md:col-span-2 mt-2 rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-950">
          <p class="mb-3 text-sm font-semibold text-zinc-800 dark:text-zinc-100">Passwort ändern (optional)</p>
        ...
        <div class="md:col-span-2 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-sm dark:border-zinc-700 dark:bg-zinc-950">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <span v-if="!(currentUser?.roles || []).length" class="text-zinc-500 dark:text-zinc-400">Keine Rolle gesetzt</span>
          </div>
        </div>
    ...
    <div
      ...
      <div class="w-full max-w-xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
        <div class="mb-4">
          <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Authenticator einrichten</h3>
          <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Scanne den QR-Code mit deiner Authenticator-App. Du kannst den Link zusätzlich erneut per E-Mail versenden.
          </p>
        </div>

        <div class="mb-4 flex justify-center rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-950">
          <img
            v-if="mfaSetupQrDataUrl"
            :src="mfaSetupQrDataUrl"
            alt="Authenticator QR-Code"
            class="h-64 w-64 rounded-lg border border-zinc-200 bg-white p-2 dark:border-zinc-700" />
          <p v-else class="text-sm text-zinc-500 dark:text-zinc-400">QR-Code konnte nicht gerendert werden.</p>
        </div>

        <details class="mb-4 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-950">
          <summary class="cursor-pointer text-sm text-zinc-700 dark:text-zinc-300">Manuelle otpauth URL anzeigen</summary>
        ...
        <div class="flex flex-wrap justify-end gap-2">
          <button
            ...
            :disabled="mfaSetupActivating"
            @click="activateAuthenticatorNow">
          ...
          <button
            ...
            :disabled="mfaSetupMailSending"
            @click="resendMfaActivationEmail">
