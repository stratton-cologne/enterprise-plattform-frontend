...
<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col gap-3 border-b border-zinc-200 px-5 py-3 md:flex-row md:items-center md:justify-between dark:border-zinc-800">
        <div class="relative w-full md:max-w-sm">
          <input
            v-model="searchTerm"
            type="text"
            placeholder="Suche nach Name, E-Mail, Rolle, Abteilung ..."
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
            ...
            <tr v-for="user in pagedUsers" :key="user.id" class="hover:bg-zinc-100/80 dark:hover:bg-zinc-800/40">
              ...
              <td class="px-4 py-3 text-zinc-700 dark:text-zinc-300">{{ user.roles.map((r) => r.name).join(', ') || 'keine' }}</td>
              <td class="px-4 py-3">
                <span class="rounded-md border border-zinc-300 px-2 py-1 text-xs text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">{{ user.mfa_method || 'email' }}</span>
              </td>
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-[90] flex items-center justify-center px-4">
      ...
      <div class="relative max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-zinc-300 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form class="grid gap-3 md:grid-cols-2" @submit.prevent="saveUser">
          ...
          <div class="md:col-span-2">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Rollen / Gruppen</p>
