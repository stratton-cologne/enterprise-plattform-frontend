...
<template>
  <div class="space-y-6">
    <section class="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div class="flex flex-col gap-3 border-b border-zinc-200 px-5 py-3 md:flex-row md:items-center md:justify-between dark:border-zinc-800">
        <div class="relative w-full md:max-w-sm">
          <input
            v-model="searchTerm"
            type="text"
            placeholder="Suche nach Rolle, Slug, Parent oder Recht ..."
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <select v-model.number="perPage" class="rounded-lg border border-zinc-300 bg-white px-2 py-2 text-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-left text-sm">
          <thead class="border-b border-zinc-200 bg-zinc-100 text-xs uppercase tracking-wide text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-400">
            <tr>
          ...
          <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
            ...
            <tr v-for="role in pagedRoles" :key="role.id" class="hover:bg-zinc-100/80 dark:hover:bg-zinc-800/40">
        ...
        <div class="flex flex-col gap-3 border-t border-zinc-200 px-4 py-3 text-sm text-zinc-600 md:flex-row md:items-center md:justify-between dark:border-zinc-800 dark:text-zinc-400">
          <div>
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="rounded-md border border-zinc-300 px-3 py-1.5 text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">Seite {{ currentPage }} / {{ totalPages }}</span>
            <button
              class="rounded-md border border-zinc-300 px-3 py-1.5 text-zinc-700 hover:bg-zinc-100 disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-[90] flex items-center justify-center px-4">
      ...
      <div class="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-zinc-300 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form class="grid gap-3 md:grid-cols-2" @submit.prevent="saveRole">
          ...
          <select v-model="form.parent_id" class="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
            <option value="">Keine übergeordnete Rolle</option>
            <option v-for="role in roles.filter((r) => r.id !== selectedRoleId)" :key="role.id" :value="role.id">
              {{ role.name }}
            </option>
          </select>

          <div class="md:col-span-2">
            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Rechte</p>
